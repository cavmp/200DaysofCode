...
def encode(plaintext):
    global rotors, reflector, ring_settings, ring_positions, plugboard
    ...
    rotorALetter = ring_positions[0]
    rotorBLetter = ring_positions[1]
    rotorCLetter = ring_positions[2]

    rotorASetting = ring_settings[0]
    offsetASetting = alphabet.index(rotorASetting)
    rotorBSetting = ring_settings[1]
    offsetBSetting = alphabet.index(rotorBSetting)
    rotorCSetting = ring_settings[2]
    offsetCSetting = alphabet.index(rotorCSetting)

    rotorA = caesar_shift(rotorA, offsetASetting)
    rotorB = caesar_shift(rotorB, offsetBSetting)
    rotorC = caesar_shift(rotorC, offsetCSetting)
...
while True:
    plaintext = input("\nEnter text to encode or decode (Press 'enter' when done using this program):\n")
    if plaintext == '':
        exit()
    else:
