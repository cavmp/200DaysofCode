...
Tops = Frame(root, width = 1350, height = 20, bd = 8, relief = "raise")
Tops.pack(side = TOP)
...
#Heading
lblInfo = Label(Tops, font  = ('arial', 50, 'bold'), text = " \t Cafe Management System \t\t", bd = 50)
...
def reset():
    ...
    E_Caramel_Macchiato.set("0")
    E_Coffee_Cakes.set("0")
    E_Red_Velvet_Cake.set("0")
    E_Black_Forest_Cake.set("0")
    E_Lemon_Cheesecake.set("0")
    E_New_York_Cheesecake.set("0")
    E_Strawberry_Cake.set("0")
    E_Tiramisu.set("0")
    E_Chocolate_Mud_Cake.set("0")
    ...
    txtLemon_Cheesecake.configure(state = DISABLED)
    txtNew_York_Cheesecake.configure(state = DISABLED)
    txtStrawberry_Cake.configure(state = DISABLED)
    txtCappuccino.configure(state = DISABLED)
    txtTiramisu.configure(state = DISABLED)
    txtChocolate_Mud_Cake.configure(state = DISABLED)
...
def chkbutton_value():
    ...
    if(var3.get() == 1):
        ...
        E_Iced_Latte.set("0")
    if(var4.get() == 1):
        txtCafe_Mocha.configure(state = NORMAL)
    elif var4.get() == 0:
        txtCafe_Mocha.configure(state = DISABLED)
        E_Cafe_Mocha.set("0")
    if(var5.get() == 1):
    ...
    if(var7.get() == 1):
        ...
        E_American_Coffee.set("0")
    if(var8.get() == 1):
        txtCaramel_Macchiato.configure(state = NORMAL)
    elif var8.get() == 0:
        txtCaramel_Macchiato.configure(state = DISABLED)
        E_Caramel_Macchiato.set("0")
    if(var9.get() == 1):
    ...
    if(var11.get() == 1):
        ...
        E_Black_Forest_Cake.set("0")
    if(var12.get() == 1):
        txtLemon_Cheesecake.configure(state = NORMAL)
    elif var12.get() == 0:
        txtLemon_Cheesecake.configure(state = DISABLED)
        E_Lemon_Cheesecake.set("0")
    if(var13.get() == 1):
        txtNew_York_Cheesecake.configure(state = NORMAL)
    elif var13.get() == 0:
        txtNew_York_Cheesecake.configure(state = DISABLED)
        E_New_York_Cheesecake.set("0")
    if(var14.get() == 1):
    ...
    if(var15.get() == 1):
        ...
        E_Tiramisu.set("0")
    if(var16.get() == 1):
        txtChocolate_Mud_Cake.configure(state = NORMAL)
    elif var16.get() == 0:
        txtChocolate_Mud_Cake.configure(state = DISABLED)
        E_Chocolate_Mud_Cake.set("0")
...
def Receipt():
    ...
    if E_Cafe_Mocha.get() > 0:
        txtReceipt.insert(END, 'Vale Coffee:\t\t\t'+str(E_Cafe_Mocha.get())+'\t\t\t'+str('₱%.2f'%(E_Cafe_Mocha.get()*1.89))+'\n')
    ...
    if E_Caramel_Macchiato.get() > 0:
        txtReceipt.insert(END, 'Caramel Macchiato:\t\t\t'+str(E_Caramel_Macchiato.get())+'\t\t\t'+str('₱%.2f'%(E_Caramel_Macchiato.get()*1.29))+'\n')
    ...
    if E_Lemon_Cheesecake.get() > 0:
        txtReceipt.insert(END, 'Lemon Cheesecake:\t\t\t'+str(E_Lemon_Cheesecake.get())+'\t\t\t'+str('₱%.2f'%(E_Lemon_Cheesecake.get()*1.49))+'\n')
    if E_New_York_Cheesecake.get() > 0:
        txtReceipt.insert(END, 'New York Cheesecake:\t\t\t'+str(E_New_York_Cheesecake.get())+'\t\t\t'+str('₱%.2f'%(E_New_York_Cheesecake.get()*1.80))+'\n')
    if E_Strawberry_Cake.get() > 0:
        txtReceipt.insert(END, 'Strawberry Cake:\t\t\t'+str(E_Strawberry_Cake.get())+'\t\t\t'+str('₱%.2f'%(E_Strawberry_Cake.get()*1.67))+'\n')
    if E_Tiramisu.get() > 0:
        txtReceipt.insert(END, 'Tiramisu:\t\t\t'+str(E_Tiramisu.get())+'\t\t\t'+str('₱%.2f'%(E_Tiramisu.get()*1.6))+'\n')
    if E_Chocolate_Mud_Cake.get() > 0:
        txtReceipt.insert(END, 'Chocolate Mud Cake:\t\t\t'+str(E_Chocolate_Mud_Cake.get())+'\t\t\t'+str('₱%.2f'%(E_Chocolate_Mud_Cake.get()*1.99))+'\n')
...
E_Caramel_Macchiato = IntVar()

E_Coffee_Cakes = IntVar()
E_Red_Velvet_Cake = IntVar()
E_Black_Forest_Cake = IntVar()
E_Lemon_Cheesecake = IntVar()
E_New_York_Cheesecake = IntVar()
E_Strawberry_Cake = IntVar()
E_Tiramisu = IntVar()
E_Chocolate_Mud_Cake = IntVar()
...
Cafe_Mocha = Checkbutton(f1aa, text="Cafe Mocha  \t\t", variable = var4, onvalue = 1, offvalue = 0, command = chkbutton_value,font  = ('arial', 18, 'bold')).grid(row = 3, sticky = W)
...
American_Coffee = Checkbutton(f1aa, text="Americano \t\t", variable = var7, onvalue = 1, offvalue = 0, command = chkbutton_value,font  = ('arial', 18, 'bold')).grid(row = 6, sticky = W)
Caramel_Macchiato = Checkbutton(f1aa, text="Caramel Macchiato  \t\t", variable = var8, onvalue = 1, offvalue = 0, command = chkbutton_value,font  = ('arial', 18, 'bold')).grid(row = 7, sticky = W)
...
Lemon_Cheesecake = Checkbutton(f1ab, text="Lemon Cheesecake\t", variable = var12, onvalue = 1, offvalue = 0, command = chkbutton_value, font  = ('arial', 18, 'bold')).grid(row = 3, sticky = W)
New_York_Cheesecake = Checkbutton(f1ab, text="New York Cheesecake\t", variable = var13, onvalue = 1, offvalue = 0, command = chkbutton_value, font  = ('arial', 18, 'bold')).grid(row = 4, sticky = W)
Strawberry_Cake = Checkbutton(f1ab, text="Strawberry Cake\t", variable = var14, onvalue = 1, offvalue = 0, command = chkbutton_value, font  = ('arial', 18, 'bold')).grid(row = 5, sticky = W)
Tiramisu = Checkbutton(f1ab, text="Tiramisu\t", variable = var15, onvalue = 1, offvalue = 0, command = chkbutton_value, font  = ('arial', 18, 'bold')).grid(row = 6, sticky = W)
Chocolate_Mud_Cake = Checkbutton(f1ab, text="Chocolate Mud Cake\t", variable = var16, onvalue = 1, offvalue = 0, command = chkbutton_value, font  = ('arial', 18, 'bold')).grid(row = 7, sticky = W)
...
txtCafe_Mocha = Entry(f1aa, font=('arial', 16, 'bold'), bd = 8, width = 6, justify = 'left', textvariable = E_Cafe_Mocha, state = DISABLED )
txtCafe_Mocha.grid(row = 3, column = 1)
...
txtCaramel_Macchiato = Entry(f1aa, font=('arial', 16, 'bold'), bd = 8, width = 6, justify = 'left', textvariable = E_Caramel_Macchiato, state = DISABLED )
txtCaramel_Macchiato.grid(row = 7, column = 1)
...
txtLemon_Cheesecake = Entry(f1ab, font=('arial', 16, 'bold'), bd = 8, width = 6, justify = 'left', textvariable = E_Lemon_Cheesecake, state = DISABLED )
txtLemon_Cheesecake.grid(row = 3, column = 1)
txtNew_York_Cheesecake = Entry(f1ab, font=('arial', 16, 'bold'), bd = 8, width = 6, justify = 'left', textvariable = E_New_York_Cheesecake, state = DISABLED )
txtNew_York_Cheesecake.grid(row = 4, column = 1)
txtStrawberry_Cake = Entry(f1ab, font=('arial', 16, 'bold'), bd = 8, width = 6, justify = 'left', textvariable = E_Strawberry_Cake, state = DISABLED )
txtStrawberry_Cake.grid(row = 5, column = 1)
txtTiramisu = Entry(f1ab, font=('arial', 16, 'bold'), bd = 8, width = 6, justify = 'left', textvariable = E_Tiramisu, state = DISABLED )
txtTiramisu.grid(row = 6, column = 1)
txtChocolate_Mud_Cake = Entry(f1ab, font=('arial', 16, 'bold'), bd = 8, width = 6, justify = 'left', textvariable = E_Chocolate_Mud_Cake, state = DISABLED )
txtChocolate_Mud_Cake.grid(row = 7, column = 1)
# Receipt Info
